...
/*	
	var t=this;
	chrome.extension.sendRequest({func:"get","n":'KOCPower_Bot'}, 
	function(response) {
	//~~~
...
SetValuesCache:function() {
	...
	document.cookie='KOCPower_Bot='+escape(JSON2.stringify(this.valuesCache))+'; expires='+
...
if(!this.GM_log || isChrome) {
	GM_log=function(m) {
		console.log(m);
	}
	GM_registerMenuCommand=function(text,f) {
	}
	GM_setValue = function(n,val){
		return ChromeFuncs.browser_setValue(n,val);
	}
	GM_getValue = function(n,def){
		return ChromeFuncs.browser_getValue(n,def);
	}
...
Tabs.transport = {
  ...
  check:false,
    ...
    init: function(div){
      ...
      m += '<TD width=180px align=left>Keep: <INPUT id=pbtargetamountAstone type=text size=11 maxlength=11 value="0" disabled=true\></td>';
      m += '<TD width=100px>Trade: <INPUT id=pbtradeamountAstone type=text size=11 maxlength=11 value="0"\></td>';
