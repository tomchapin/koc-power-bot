...
//logit ("NOW="+ now + ' alarmActive='+ Options.alertSound.alarmActive + ' expireTime='+ Options.alertSound.expireTime);
  ...
  movetroops:function (city) {
	...
	var totalscouts = parseInt(Seed.units[cityString]['unt' + '3']);
    var totalcav = parseInt(Seed.units[cityString]['unt' + '7']);
     
     
	
	var sortem = [];
     for (var k in Seed.wilderness[cityString])
        sortem.push (Seed.wilderness[cityString][k]);
	...
	var wildx = wild.xCoord;
     var wildy = wild.yCoord;
	...
	if (totalscouts > maxsend)
	     	
		sendscouts = maxsend;
	     
	else 
	     
		sendscouts = totalscouts;
	     
	
	if (sendscouts < maxsend) {
	   		
	if (totalcav > (maxsend - sendscouts))
	   			
		sendcav = (maxsend - sendscouts);
...
if(rslt.updateSeed){update_seed(rslt.updateSeed)};
} 
else {
	logerror = 1;
	logit('FAIL :' + logerror);
...
onFailure: function () {}
	...
	logit(logerror);
	if (logerror != 1){
	totalscouts = totalscouts - sendscouts;
	totalcav = totalcav - sendcav;
	}
		
	logit('TOTAL -> S :' + sendscouts + '/' + totalscouts + ' - Cav :' + sendcav + '/' + totalcav);
	
	if (totalscouts == 0 && totalcav ==0)
		check = 1;
	}
	...
	totalscouts = parseInt(Seed.units[cityString]['unt' + '3']);
   	totalcav = parseInt(Seed.units[cityString]['unt' + '7']);
    
	logit(totalscouts + ' - ' + totalcav);
...
Tabs.Test = {
  ...
  init : function (div){
    ...
    var m = '<TABLE><TR><TD align=right>Scout: </td><TD><INPUT type=checkbox id=pbfakeIsScout></td></tr>\
        ...
        <TR><TD align=right>City: </td><TD><INPUT type=text size=6 value=5000 id=pbfakeMilitia></td></tr>\
  ...
  clickFakeAttack : function (){
    ...
    t.createFakeAttack (mil, isScout, isWild, isFalse, secs, 5000);
  },
}
...
Tabs.trader = {
  ...
  tradeRoutes: [],
	...
	t.readTradeRoutes();
		...
		m += '<TD><INPUT id=pbDelRoutes type=submit value="Delete Routes"></td>';
        m += '</tr></table></div>';
		...
		m += '<TD>MOVE</td><TD><INPUT id=pbtradeamount type=text size=8 maxlength=8 value="MAX"\></td>';
        ...
        document.getElementById('pbDelRoutes').addEventListener('click', function(){
            t.delTradeRoutes();
        }, false);
	...
	//setInterval (t.e_tradeRoutes, 20000);
    ...
    doTrades: function(count){
    	...
    	var trade_amount = t.tradeRoutes[count]["trade_amount"];
    	if (trade_amount == 'MAX') trade_amount=0;
    	    	alert(trade_amount);
    	
    	var target_amount = t.tradeRoutes[count]["target_amount"]; 
    	var resource_type = t.tradeRoutes[count]["res_type"];
		...
		for (var t=0; t< Seed.cities.length;t++) {
 		...
 		switch (resource_type) {
			case 'gold': params.gold = carry_amount;break;
			case 'food': 
				citymax = parseInt(Seed.resources[cityID]['rec1'][0] / 3600);
				carry_amount = (citymax - target_amount);
          		if (trade_amount != 0 && trade_amount < carry_amount) carry_amount = trade_amount;
          		if (maxload < carry_amount) carry_amount = maxload;
          		alert(trade_amount + ' / ' + citymax + ' / ' + carry_amount);
				params.r1 = carry_amount;
          		break;
		case 'wood': 
			citymax = parseInt(Seed.resources[cityID]['rec2'][0] / 3600);
			carry_amount = (citymax - target_amount);
			if (trade_amount != 0 && trade_amount < carry_amount) carry_amount = trade_amount;
          	if (maxload < carry_amount) carry_amount = maxload;
          	alert(trade_amount + ' / ' + citymax + ' / ' + carry_amount);
			params.r2 = carry_amount;
          	break;
		case 'stone': 
			citymax = parseInt(Seed.resources[cityID]['rec3'][0] / 3600);
			carry_amount = (citymax - target_amount);
			if (trade_amount != 0 && trade_amount < carry_amount) carry_amount = trade_amount;
         	if (maxload < carry_amount) carry_amount = maxload;
          	alert(trade_amount + ' / ' + citymax + ' / ' + carry_amount);
			params.r3 = carry_amount;
          	break;
		case 'ore': 
			citymax = parseInt(Seed.resources[cityID]['rec4'][0] / 3600);
			carry_amount = (citymax - target_amount);
          	if (trade_amount != 0 && trade_amount < carry_amount) carry_amount = trade_amount;
          	if (maxload < carry_amount) carry_amount = maxload;
          	alert(trade_amount + ' / ' + citymax + ' / ' + carry_amount);
          	if (carry_amount < 0) carry_amount=0;
			params.r4 = carry_amount;
          	break;
		}	
		
		
		wagons_needed = (carry_amount / maxloadperwagon);
		wagons_needed = wagons_needed.toFixed(0);	
		if (wagons_needed < (carry_amount / maxloadperwagon))
			wagons_needed++;	
		...
		logit('Before send');
   		if (carry_amount > 0) {
          logit('Trade   From: ' + cityname + "   To: " + xcoord + ',' + ycoord + "    -> Resource (" + resource_type + "): " + carry_amount + "   - Wagons: " + wagons_needed);
      		new AjaxRequest(unsafeWindow.g_ajaxpath + "ajax/march.php" + unsafeWindow.g_ajaxsuffix, {
                  method: "post",
                  parameters: params,
                  loading: true,
