...
Tabs.trader = {
  ...
  tradeRoutes: [],
    ...
    init: function(div){
	    ...
	    window.addEventListener('unload', t.onUnload, false);
	    setInterval (t.e_tradeRoutes, 10000);

	},
	e_tradeRoutes: function(){
        var t = Tabs.trader;
        var lastTrade = 0;
        if ((unixTime() - lastTrade) > 10000){
        	t.doTrades();
        	lastTrade = unixTime();
        }
    },
    ...
    doTrades: function(){
    	...
    	var resource_type;
       	...
       	//alert(t.tradeRoutes.length);
       	
       	city = t.tradeRoutes[0]["city"];
        xcoord = t.tradeRoutes[0]["target_x"];
       	xcoord = t.tradeRoutes[0]["target_y"];
       	trade_amount = t.tradeRoutes[0]["trade_amount"];
        resource_type = t.tradeRoutes[0]["res_type"];
       
        var cityID = 'city' + city;
        var wagons =  parseInt(Seed.units[cityID]['unt'+9]);
        //var test = Cities.cities[city][1];
        //var wagons = test.numwagons;
