...
Tabs.tower = {
    ...
    init: function(div){
		...
		for (var i = 0; i < Cities.cities.length; i++) {
            t["aQ_" + Cities.cities[i].id] = JSON2.parse(GM_getValue('aQ_' + getServerId() + '_' + Cities.cities[i].id, '[]'));
			if (typeof t["aQ_" + Cities.cities[i].id] == 'undefined' || (t["aQ_" + Cities.cities[i].id]) == "") {
				t["aQ_" + Cities.cities[i].id] = [];
			}
        }
        t.readAlertState();

        var m = '<DIV id=pbTowrtDivF class=ptstat>TOWER FUNCTIONS</div><TABLE id=pbtowerfunctions width=100% height=0% class=pbTab><TR>';
        if (t.towerStates.running == false) {
        ...
        m += '</tr><TR>';
		for (var i = 0; i < Cities.cities.length; i++) {
			m += '<TD><INPUT id=pbattackqueue_' + Cities.cities[i].id + ' type=submit value="(x/y) : a | b | c"></td>';
        }
        m += '</tr><TR>';
        m += '</tr></table></div>';
	...
	e_checkTower: function(){
		...
		if (matTypeof(Seed.queue_atkinc) != 'array'){
			for (var k in Seed.queue_atkinc){
				...
				if ((m.marchType==3 || m.marchType==4) && parseIntNan(m.arrivalTime)>now){
					t.updateAttackQueue (m);
					if (t.towerStates.running == true) {
        ...
        setTimeout(t.e_checkTower, 10000);
    },
	getCitySlotState: function (m) {
		var t = Tabs.tower;
      ...
      if (emb.count > 0){
		...
		t.setCitySlotState(m.toCityId, availSlots, emb.maxLevel);
      }
	},
	setCitySlotState: function (cityId, availSlots, maxSlots) {
		document.getElementById('pbattackqueue_'+ cityId).value = "( " + availSlots +' / ' + maxSlots + ') : a | b | c | d';
	},
	storeTowerMarch : function (m){
		var t = TowerAlerts;
		var now = unixTime();
	...
	updateAttackQueue : function (m, force){
		var t = TowerAlerts;
    ...
    saveBuildStates: function(){
        ...
        GM_setValue('towerStates_' + serverID, JSON2.stringify(t.towerStates));
    },
	...
	saveAlertState: function(){
		var t = Tabs.tower;
        var serverID = getServerId();
        GM_setValue('alertState_' + serverID, JSON2.stringify(t.towerStates));
    },
    readAlertState: function(){
        ...
        if (s != null) {
            state = JSON2.parse(s);
            for (k in state) 
                t.towerStates[k] = state[k];
        }
    },
    toggleAlertState: function(obj){
		var t = Tabs.tower;
        if (t.towerStates.running == true) {
            t.towerStates.running = false;
            t.saveAlertState();
            obj.value = "Audio Alert = OFF";
        }
        else {
            t.towerStates.running = true;
            t.saveAlertState();
...
Tabs.build = {
    ...
    e_autoBuild: function(){
      ...
      if (t.buildStates.running == true) {
          for (var i = 0; i < Cities.cities.length; i++) {
              var cityId = Cities.cities[i].id;
              if (matTypeof(Seed.queue_con["city" + cityId])=='object' && Seed.queue_con["city" + cityId][0]!=null) {
